...
def load_ticket_data(
    ...
    *,
    ...
    while True:
        ...
        multi_sample_df = multi_sample_df[
            ~multi_sample_df[topic_col].isin(too_few_classes)
        ]
    ...
    if len(multi_sample_df) > 0:
        ...
        if isinstance(test_size, int):
            if split_test_size < n_classes:
                print(
                    f"[WARN] test_size={test_size} < n_classes={n_classes}. Setting test_size=n_classes."
                )
                split_test_size = n_classes
            if split_test_size >= n_samples:
                raise ValueError(
                    f"test_size={test_size} is greater than total samples={n_samples}."
                )
        elif isinstance(test_size, float):
            if int(split_test_size * n_samples) < n_classes:
                min_size = n_classes / n_samples + 0.01
                print(
                    f"[WARN] test_size={test_size} too small for n_classes={n_classes}. Setting test_size={min_size:.2f}."
                )
